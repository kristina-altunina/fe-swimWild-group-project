...
export default function HomeScreen({ navigation }) {
    ...
    if (isGranted) {
      Location.getCurrentPositionAsync({})
        .then(({ coords }) => {
          ...
          return instance.get(
            `locations?lat=${coords.latitude}&lon=${coords.longitude}`
          );
        })
        .then(({ data }) => {
      ...
      setRegion({
        latitude: 54.636, // UK latitude
        longitude: -3.3631, // UK longitude
        latitudeDelta: 10,
        longitudeDelta: 10,
      });
      instance
        .get(`locations?lat=${region.latitude}&lon=${region.longitude}&limit=6`)
        .then(({ data }) => {
          setLocations(data);
  ...
  return (
    <View style={styles.container}>
      ...
      <GoogleMapComponent region={region} onRegionChange={handleRegionChange}>
        {userLocation && (
          <Marker
            coordinate={{
